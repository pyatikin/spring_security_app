# Spring Security Application with Keycloak and MTLS

Защищенное веб-приложение с Spring Boot, Spring Security, Keycloak аутентификацией и MTLS через NGINX.
...
- **Права администратора** (для редактирования hosts файла)
...
Дождитесь запуска всех сервисов (~2-3 минуты). Проверьте статус:
```bash
docker-compose ps
```
...
#### Firefox:
1. Настройки → Приватность и защита → Сертификаты → Просмотр сертификатов
2. **Центры сертификации** → Импорт → `certs/ca.crt` → Доверять
3. **Ваши сертификаты** → Импорт → `certs/client.p12` (пароль: `changeit`)
...
```
┌──────────┐
│  Browser │ ← Клиентский сертификат
└────┬─────┘
     │ HTTPS + MTLS
     ▼
┌──────────┐
│  NGINX   │ ← Проверка сертификата, логирование DN
└────┬─────┘
     │
     ├──────────┬──────────┐
     │          │          │
...
- **Spring Security 6** с OAuth2/OIDC
- **Keycloak 23.0** для аутентификации
- **PostgreSQL 16** для Keycloak
- **NGINX** с MTLS
- **Docker Compose**

### Безопасность

1. **MTLS на NGINX** - требуется клиентский сертификат
2. **OAuth2/OIDC** - через Keycloak
3. **CSRF защита** - включена
4. **CORS политики** - ограниченные origins
5. **Role-Based Access** - роли USER и ADMIN
